...
| Explicit grid, environment hints | Useful experiment without query delays, raw mode, stdin consumption, or premature auto-layout thresholds. Hints are not capability verification. Text wins on pipes. |
...
Automated tests decode emitted payloads and model the cursor subset; visible
terminal behavior is still untested. Anonymous inline images avoid ID reuse across
